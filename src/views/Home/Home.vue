    ...
    <el-col :span="8">
      ...
      <el-card shadow="hover" style="height:522px;margin-top:20px;">
        <el-table :data="tableData">
          <el-table-column show-overflow-tooltip v-for="(val, idx) in tableLabel" :key="idx" :prop="idx" :label="val"></el-table-column>
        </el-table>
      </el-card>
    </el-col>
    <el-col :span="16">
  ...
  methods: {
    getTableData() {
      this.$http.get('/home/getData').then(res => {
        ...
        this.echartData.order.xData = order.date // x轴  数据
        // 第一步 去除series 中的name部分 ---键名
        let keyArray = Object.keys(order.data[0])
        // console.log(keyArray)
        keyArray.forEach(key => {
          this.echartData.order.series.push({
            name: key == 'weichat ' ? '微信小程序' : key,
